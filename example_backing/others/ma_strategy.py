...
class BackTest(object):
    ...
    def trade_test(self, data, n_list):
        ...
        for i in range(n_1, len(data)):
            ...
            price_list_2 = priceList[-n_2:]
            # MA
            long_ma = priceList.mean()
            short_ma = price_list_2.mean()
            # 判断交易方向
            if self.status == 0:
                # 上穿
                if short_ma > long_ma:
...
def run():
    ...
    data = bt.db(cycle=1, startTime=s, endTime=e, cypair=symbol, table_name='bi_an_copy')
    ...
    with open(r"C:\Users\Administrator\Desktop\1.txt", "a") as f:
        f.write("\n" + symbol + "\n" + str(x.tolist()) + "\n" + str(y.tolist()))
    plt.bar(x, y)
    for a, b in zip(x, y):
